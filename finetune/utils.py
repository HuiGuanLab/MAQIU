...
def set_weight_decay_blip(model, lr_backbone=0.0000125, lr_head=1e-4, backbone_weight_decay=0.01, head_weight_decay=1e-4, norm_weight_decay=0.0):
    ...
    for name, module in model.named_modules():
        for param_name, p in module.named_parameters(recurse=False):
            ...
            if 'blip_backbone' in full_name:
                if isinstance(module, norm_layers):
                    groups["backbone_norm"].append(p)
                else:
                    groups["backbone_decay"].append(p)
            else:
                if isinstance(module, norm_layers):
                    groups["head_norm"].append(p)
                else:
                    groups["head_decay"].append(p)
    param_groups = []
    if groups["backbone_decay"]:
        param_groups.append({"params": groups["backbone_decay"], "lr": lr_backbone, "weight_decay": backbone_weight_decay})
    if groups["backbone_norm"]:
        param_groups.append({"params": groups["backbone_norm"], "lr": lr_backbone, "weight_decay": norm_weight_decay})
    if groups["head_decay"]:
        param_groups.append({"params": groups["head_decay"], "lr": lr_head, "weight_decay": head_weight_decay})
    if groups["head_norm"]:
        param_groups.append({"params": groups["head_norm"], "lr": lr_head, "weight_decay": norm_weight_decay})
...
def clip_load(name: str):

    if name in _MODELS:
        model_path = _download(_MODELS[name], os.path.expanduser("logs/cache/clip"))
